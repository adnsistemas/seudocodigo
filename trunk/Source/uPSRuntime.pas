...
interface
uses
  SysUtils, classes, uPSUtils{$IFDEF DELPHI6UP}, variants{$ENDIF}{$IFNDEF PS_NOIDISPATCH}{$IFDEF DELPHI3UP}, ActiveX, Windows{$ELSE}, Ole2{$ENDIF}{$ENDIF};
...
{$ENDIF}
  ...
  TPSOnReadEvent = procedure (Sender: TPSExec; t: TPsBaseType;var Data);
  TPSOnShowEvent = procedure (Sender: TPSExec; t: TPsBaseType;const Data);
  TPSOnShowReadEvent = procedure (Sender: TPSExec;const Data:string);
  ...
  Protected
    ...
    FOnRead: TPSOnReadEvent;

    FOnShow: TPSOnShowEvent;
    FOnShowRead: TPSOnShowReadEvent;
  ...
  Public
    ...
    property OnRead: TPSOnReadEvent read FOnRead write FOnRead;
    property OnShow: TPSOnShowEvent read FOnShow write FOnShow;
    property OnShowForRead: TPSOnShowReadEvent read FOnShowRead write FOnShowRead;
...
{$ENDIF}
    ...
    procedure SetChar(ItemNo: Longint; const Data: char);
...
{$ENDIF}
    procedure SetReal(ItemNo: Longint; const Data: real);
...
{$ENDIF}
procedure PSSetReal(Src: Pointer; aType: TPSTypeRec; var Ok: Boolean; const Val: real);
...
{$ENDIF}
procedure VNSetReal(const Src: TPSVariantIFC; const Val: real);
...
{$ENDIF}
procedure VSetReal(const Src: PIFVariant; const Val: real);
...
{estructura utilizada para almacenar cada uno de los elementos de las listas}
type
...
implementation
uses
  langdef, math, TypInfo {$IFDEF DELPHI3UP}{$IFNDEF FPC} , ComObj {$ENDIF}{$ENDIF};
...
const
{$ENDIF }
  RPS_InvalidVariantRef = 'Referencia a ' + CS_variant + ' no válida';
  RS_Computation_vmp_fail = 'Falló el cómputo del método virtual: t<>Ptr';

  RPS_UnknownIdentifier = 'Identificador desconocido';
  RPS_Exception = 'Excepción: %s';
  RPS_Invalid = '[No válido]';

  //- PSErrorToString
  RPS_NoError = 'Sin error';
  RPS_CannotImport = 'No se puede importar %s';
  RPS_InvalidType = 'Tipo no válido';
  RPS_InternalError = 'Error interno';
  RPS_InvalidHeader = 'Cabecera no válida';
  RPS_InvalidOpcode = 'Código de operación no válido';
  RPS_InvalidOpcodeParameter = 'Parámetro de operación no válido';
  RPS_NoMainProc = 'No hay procedimiento principal';
  RPS_OutOfGlobalVarsRange = 'Se salió del rango de variables globales';
  RPS_OutOfProcRange = 'Se salió del rango de procedimientos';
  RPS_OutOfRange = 'Fuera de rango';
  RPS_OutOfStackRange = 'Fuera de rango de pila';
  RPS_TypeMismatch = 'Incoherencia de tipos';
  RPS_UnexpectedEof = 'Fin de archivo inesperado';
  RPS_VersionError = 'Error de versión';
  RPS_DivideByZero = 'División por cero';
  RPS_MathError = 'Error matemático';
  RPS_CouldNotCallProc = 'No se pudo invocar procedimiento';
  RPS_OutofRecordRange = 'Fuera del rango del registro';
  RPS_NullPointerException = 'Excepción de puntero nulo';
  RPS_NullVariantError = 'Error de ' + CS_variant + ' nulo';
  RPS_OutOfMemory = 'Sin memoria';
  RPS_InterfaceNotSupported = 'Interface no soportada';
  RPS_UnknownError = 'Error desconocido';


  RPS_InvalidVariable = 'Variable no válida';
  RPS_InvalidArray = 'Arreglo no válido';
  RPS_OLEError = 'Error OLE %.8x';
  RPS_UnknownProcedure = 'Procedimiento desconocido';
  RPS_NotEnoughParameters = 'Parámetros insuficientes';
  RPS_InvalidParameter = 'Parámetro no válido';
  RPS_TooManyParameters = 'Demasiados parámetros';
  RPS_OutOfStringRange = 'Fuera de rango de la cadena';
  RPS_CannotCastInterface = 'No se puede forzar el tipo de una interface';
  RPS_CannotCastObject = 'No se pudo forzar el tipo de un objeto';
  RPS_CapacityLength = 'Capacidad < Longitud';
  RPS_CanOnlySendLastItem = 'Sólo se puede quitar el último elemento de una pila';
  RPS_NILInterfaceException = 'Interface nula';
  RPS_UnknownMethod = 'Método desconocido';
...
begin
  ...
  begin
    ...
    pp := GetPropInfo(PTypeInfo(Instance.ClassInfo), GetDelphiPropertyName(string(s)));
    ...
    case pp^.PropType^.Kind of
      ...
      tkSet: begin Result := '[' + CS_set + ']'; exit; end;
      tkClass: begin Instance := TObject(GetOrdProp(Instance, pp)); end;
      tkMethod: begin Result := '[' + CS_Method + ']'; exit; end;
      tkVariant: begin Result := '[' + CS_Variant + ']'; exit; end;
	  {$IFDEF DELPHI6UP}
	  {$IFNDEF PS_NOWIDESTRING}tkWString: begin Result := ''''+tbtString(GetWideStrProp(Instance, pp))+''; end; {$ENDIF}
	  {$ENDIF}
      else begin Result := '[' + CS_Unknown + ']'; exit; end;
    end;
    if Instance = nil then begin result := CS_nil; exit; end;
  end;
  Result := tbtstring(Instance.ClassName);
end;
...
begin
  ...
  begin
    try
      if TVarData(p.Dta^).VType = varDispatch then
        Result := CS_Variant + '(' + CS_IDispatch + ')'
  ...
  case p.aType.BaseType of
    btProcptr: begin Result := CS_Proc + ': '+inttostr(tbtu32(p.Dta^)); end;
...
begin
  Result := 'RemObjects Pascal Script. Copyright (c) 2004 by RemObjects Software - Adaptación a seudocódigo por FRM - UTN';
end;
...
{$ENDIF}
    ...
    btSingle: result := Trunc(tbtsingle(Src^));
    btDouble: result := Trunc(tbtdouble(Src^));
    btExtended: result := Trunc(tbtextended(Src^));
    btCurrency: result := Trunc(tbtcurrency(Src^));
...
begin
  ...
  c := val;
  case aType.BaseType of
    btSingle: tbtSingle(src^) := s;
    btDouble: tbtDouble(src^) := d;
    btCurrency: tbtCurrency(src^) := c;
    btExtended: tbtExtended(src^) := Val;
    btVariant:
      begin
        try
    ...
    case desttype.BaseType of
      ...
      btU32:
        ...
        {$IFNDEF PS_NOWIDESTRING}    btWideChar: tbtu32(Dest^) := Ord(tbtwidechar(Src^));{$ENDIF}
            ...
            btSingle: tbtu32(Dest^) := Trunc(tbtsingle(Src^));
            btDouble: tbtu32(Dest^) := Trunc(tbtdouble(Src^));
            btExtended: tbtu32(Dest^) := Trunc(tbtextended(Src^));
            btCurrency: tbtu32(Dest^) := Trunc(tbtcurrency(Src^));
            else raise Exception.Create(RPS_TypeMismatch);
          end;
        end;
      btS32:
        ...
        {$IFNDEF PS_NOWIDESTRING}  btWideChar: tbts32(Dest^) := Ord(tbtwidechar(Src^));{$ENDIF}
            ...
            btSingle: tbts32(Dest^) := Trunc(tbtsingle(Src^));
            btDouble: tbts32(Dest^) := Trunc(tbtdouble(Src^));
            btExtended: tbts32(Dest^) := Trunc(tbtextended(Src^));
            btCurrency: tbts32(Dest^) := Trunc(tbtcurrency(Src^));
            else raise Exception.Create(RPS_TypeMismatch);
          end;
        end;
      {$IFNDEF PS_NOINT64}
      ...
      btVariant:
        begin
{$IFNDEF PS_NOINTERFACES}
          if srctype.ExportName = CS_IDISPATCH then
          begin
            {$IFDEF DELPHI3UP}
...
{$IFNDEF PS_NOINTERFACES}
      btInterface:
        begin
          if Srctype.BaseType = btVariant then
          begin
            if desttype.ExportName = CS_IDISPATCH then
            begin
              {$IFDEF Delphi3UP}
...
begin
  try
    ...
    11: {^}
      begin
          case var1Type.BaseType of
            btU8: tbtU8(var1^) := Trunc(IntPower(tbtU8(var1^),PSGetUInt(Var2, var2type)));
            btS8: tbts8(var1^) := Trunc(IntPower(tbts8(var1^),PSGetInt(Var2, var2type)));
            btU16: tbtU16(var1^) := Trunc(IntPower(tbtU16(var1^),PSGetUInt(Var2, var2type)));
            btS16: tbts16(var1^) := Trunc(IntPower(tbts16(var1^),PSGetInt(Var2, var2type)));
            btU32:
              begin
                if var2type.BaseType = btPointer then
                begin
                ...
                case var2type.BaseType of
                  btU8: tbtU32(var1^) := Trunc(IntPower(tbtU32(var1^),tbtu8(var2^)));
                  btS8: tbtU32(var1^) := Trunc(IntPower(tbtU32(var1^),cardinal(longint(tbts8(var2^)))));
                  btU16: tbtU32(var1^) := Trunc(IntPower(tbtU32(var1^),tbtu16(var2^)));
                  btS16: tbtU32(var1^) := Trunc(IntPower(tbtU32(var1^),cardinal(longint(tbts16(var2^)))));
                  btU32: tbtU32(var1^) := Trunc(IntPower(tbtU32(var1^),tbtu32(var2^)));
                  btS32: tbtU32(var1^) := Trunc(IntPower(tbtU32(var1^),cardinal(tbts32(var2^))));
              {$IFNDEF PS_NOINT64} btS64: tbtU32(var1^) := Trunc(IntPower(tbtU32(var1^),tbts64(var2^)));{$ENDIF}
                  btChar: tbtU32(var1^) := Trunc(IntPower(tbtU32(var1^),Ord(tbtchar(var2^))));
              {$IFNDEF PS_NOWIDESTRING}    btWideChar: tbtU32(var1^) := Trunc(IntPower(tbtU32(var1^),Ord(tbtwidechar(var2^))));{$ENDIF}
                  btVariant: tbtU32(var1^) := Trunc(IntPower(tbtU32(var1^),Variant(var2^)));
                  else raise Exception.Create(RPS_TypeMismatch);
                end;
              end;
            btS32:
              begin
                ...
                case var2type.BaseType of
                  btU8: tbts32(var1^) := Trunc(IntPower(tbts32(var1^),tbtu8(var2^)));
                  btS8: tbts32(var1^) := Trunc(IntPower(tbts32(var1^),tbts8(var2^)));
                  btU16: tbts32(var1^) := Trunc(IntPower(tbts32(var1^),tbtu16(var2^)));
                  btS16: tbts32(var1^) := Trunc(IntPower(tbts32(var1^),tbts16(var2^)));
                  btU32: tbts32(var1^) := Trunc(IntPower(tbts32(var1^),Longint(tbtu32(var2^))));
                  btS32: tbts32(var1^) := Trunc(IntPower(tbts32(var1^),tbts32(var2^)));
              {$IFNDEF PS_NOINT64} btS64: tbts32(var1^) := Trunc(IntPower(tbts32(var1^),tbts64(var2^)));{$ENDIF}
                  btChar: tbts32(var1^) := Trunc(IntPower(tbts32(var1^),Ord(tbtchar(var2^))));
              {$IFNDEF PS_NOWIDESTRING}    btWideChar: tbts32(var1^) := Trunc(IntPower(tbts32(var1^),Ord(tbtwidechar(var2^))));{$ENDIF}
                  btVariant: tbts32(var1^) := Trunc(IntPower(tbts32(var1^),Variant(var2^)));
                  else raise Exception.Create(RPS_TypeMismatch);
                end;
              end;
           {$IFNDEF PS_NOINT64}
            btS64: tbts64(var1^) := Trunc(IntPower(tbts64(var1^),PSGetInt64(var2, var2type)));
           {$ENDIF}
            btSingle:
              begin
                if var2type.BaseType = btPointer then
                ...
                case var2type.BaseType of
                  btU8: tbtsingle(var1^) := Power(tbtsingle(var1^),tbtu8(var2^));
                  btS8: tbtsingle(var1^) := Power(tbtsingle(var1^),tbts8(var2^));
                  btU16: tbtsingle(var1^) := Power(tbtsingle(var1^),tbtu16(var2^));
                  btS16: tbtsingle(var1^) := Power(tbtsingle(var1^),tbts16(var2^));
                  btU32: tbtsingle(var1^) := Power(tbtsingle(var1^),tbtu32(var2^));
                  btS32: tbtsingle(var1^) := Power(tbtsingle(var1^),tbts32(var2^));
              {$IFNDEF PS_NOINT64}    btS64: tbtsingle(var1^) := Power(tbtsingle(var1^),tbts64(var2^));{$ENDIF}
                  btSingle: tbtsingle(var1^) := Power(tbtsingle(var1^) , tbtsingle(var2^));
                  btDouble: tbtsingle(var1^) := Power(tbtsingle(var1^) , tbtdouble(var2^));
                  btExtended: tbtsingle(var1^) := Power(tbtsingle(var1^), tbtextended(var2^));
                  btCurrency: tbtsingle(var1^) := Power(tbtsingle(var1^),tbtcurrency(var2^));
                  btVariant:  tbtsingle(var1^) := Power(tbtsingle(var1^),Variant(var2^));
                  else raise Exception.Create(RPS_TypeMismatch);
                end;
              end;
            btCurrency:
              begin
                ...
                case var2type.BaseType of
                  btU8: tbtcurrency(var1^) := Power(tbtcurrency(var1^) , tbtu8(var2^));
                  btS8: tbtcurrency(var1^) := Power(tbtcurrency(var1^) , tbts8(var2^));
                  btU16: tbtcurrency(var1^) := Power(tbtcurrency(var1^) , tbtu16(var2^));
                  btS16: tbtcurrency(var1^) := Power(tbtcurrency(var1^) , tbts16(var2^));
                  btU32: tbtcurrency(var1^) := Power(tbtdouble(var1^) , tbtu32(var2^));
                  btS32: tbtcurrency(var1^) := Power(tbtcurrency(var1^) , tbts32(var2^));
              {$IFNDEF PS_NOINT64}    btS64: tbtcurrency(var1^) := Power(tbtdouble(var1^) , tbts64(var2^));{$ENDIF}
                  btSingle: tbtcurrency(var1^) := Power(tbtcurrency(var1^) , tbtsingle(var2^));
                  btDouble: tbtcurrency(var1^) := Power(tbtcurrency(var1^) , tbtdouble(var2^));
                  btExtended: tbtcurrency(var1^) := Power(tbtcurrency(var1^) , tbtextended(var2^));
                  btCurrency: tbtcurrency(var1^) := Power(tbtcurrency(var1^) , tbtcurrency(var2^));
                  btVariant:  tbtcurrency(var1^) := Power(tbtcurrency(var1^) ,  Variant(var2^));
                  else raise Exception.Create(RPS_TypeMismatch);
                end;
              end;
            btDouble:
              begin
                ...
                case var2type.BaseType of
                  btU8: tbtdouble(var1^) := Power(tbtdouble(var1^) , tbtu8(var2^));
                  btS8: tbtdouble(var1^) := Power(tbtdouble(var1^) , tbts8(var2^));
                  btU16: tbtdouble(var1^) := Power(tbtdouble(var1^) , tbtu16(var2^));
                  btS16: tbtdouble(var1^) := Power(tbtdouble(var1^) , tbts16(var2^));
                  btU32: tbtdouble(var1^) := Power(tbtdouble(var1^) , tbtu32(var2^));
                  btS32: tbtdouble(var1^) := Power(tbtdouble(var1^) , tbts32(var2^));
              {$IFNDEF PS_NOINT64}    btS64: tbtdouble(var1^) := Power(tbtdouble(var1^) , tbts64(var2^));{$ENDIF}
                  btSingle: tbtdouble(var1^) := Power(tbtdouble(var1^) , tbtsingle(var2^));
                  btDouble: tbtdouble(var1^) := Power(tbtdouble(var1^) , tbtdouble(var2^));
                  btExtended: tbtdouble(var1^) := Power(tbtdouble(var1^) , tbtextended(var2^));
                  btCurrency: tbtdouble(var1^) := Power(tbtdouble(var1^) , tbtcurrency(var2^));
                  btVariant:  tbtdouble(var1^) := Power(tbtdouble(var1^) ,  Variant(var2^));
                  else raise Exception.Create(RPS_TypeMismatch);
                end;
              end;
            btExtended:
              begin
                ...
                case var2type.BaseType of
                  btU8: tbtextended(var1^) := Power(tbtextended(var1^) , tbtu8(var2^));
                  btS8: tbtextended(var1^) := Power(tbtextended(var1^) , tbts8(var2^));
                  btU16: tbtextended(var1^) := Power(tbtextended(var1^) , tbtu16(var2^));
                  btS16: tbtextended(var1^) := Power(tbtextended(var1^) , tbts16(var2^));
                  btU32: tbtextended(var1^) := Power(tbtextended(var1^) , tbtu32(var2^));
                  btS32: tbtextended(var1^) := Power(tbtextended(var1^) , tbts32(var2^));
              {$IFNDEF PS_NOINT64}    btS64: tbtextended(var1^) := Power(tbtextended(var1^) , tbts64(var2^));{$ENDIF}
                  btSingle: tbtextended(var1^) := Power(tbtextended(var1^) , tbtsingle(var2^));
                  btDouble: tbtextended(var1^) := Power(tbtextended(var1^) , tbtdouble(var2^));
                  btExtended: tbtextended(var1^) := Power(tbtextended(var1^) , tbtextended(var2^));
                  btCurrency: tbtextended(var1^) := Power(tbtextended(var1^) , tbtcurrency(var2^));
                  btVariant:  tbtextended(var1^) := Power(tbtextended(var1^) ,  Variant(var2^));
                  else raise Exception.Create(RPS_TypeMismatch);
                end;
              end;
            btVariant:
              begin
                ...
                begin
                  if VarIsFloat(variant(var1^)) then
                    Variant(var1^) := Power(Variant(var1^) , tvar)
                  else
                    Variant(var1^) := integer(Trunc(IntPower(Variant(var1^), tvar)));
                end;
              end;
          else begin
              CMD_Err(erTypeMismatch);
           ...
           {$ENDIF}
            btSingle:
              begin
                ...
                case var2type.BaseType of
                  btU8: tbtsingle(var1^) := Trunc(tbtsingle(var1^)) div tbtu8(var2^);
                  btS8: tbtsingle(var1^) := Trunc(tbtsingle(var1^)) div tbts8(var2^);
                  btU16: tbtsingle(var1^) := Trunc(tbtsingle(var1^)) div tbtu16(var2^);
                  btS16: tbtsingle(var1^) := Trunc(tbtsingle(var1^)) div tbts16(var2^);
                  btU32: tbtsingle(var1^) := Trunc(tbtsingle(var1^)) div tbtu32(var2^);
                  btS32: tbtsingle(var1^) := Trunc(tbtsingle(var1^)) div tbts32(var2^);
              {$IFNDEF PS_NOINT64}    btS64: tbtsingle(var1^) := Trunc(tbtsingle(var1^)) div tbts64(var2^);{$ENDIF}
                  btVariant:  tbtsingle(var1^) := Trunc(tbtsingle(var1^)) div  integer(Variant(var2^));
                  else raise Exception.Create(RPS_TypeMismatch);
                end;
              end;
            btCurrency:
              begin
                ...
                case var2type.BaseType of
                  btU8: tbtcurrency(var1^) := Trunc(tbtcurrency(var1^)) div tbtu8(var2^);
                  btS8: tbtcurrency(var1^) := Trunc(tbtcurrency(var1^)) div tbts8(var2^);
                  btU16: tbtcurrency(var1^) := Trunc(tbtcurrency(var1^)) div tbtu16(var2^);
                  btS16: tbtcurrency(var1^) := Trunc(tbtcurrency(var1^)) div tbts16(var2^);
                  btU32: tbtcurrency(var1^) := Trunc(tbtcurrency(var1^)) div tbtu32(var2^);
                  btS32: tbtcurrency(var1^) := Trunc(tbtcurrency(var1^)) div tbts32(var2^);
              {$IFNDEF PS_NOINT64}    btS64: tbtcurrency(var1^) := Trunc(tbtcurrency(var1^)) div tbts64(var2^);{$ENDIF}
                  btVariant:  tbtcurrency(var1^) := Trunc(tbtcurrency(var1^)) div  integer(Variant(var2^));
                  else raise Exception.Create(RPS_TypeMismatch);
                end;
              end;
            btDouble:
              begin
                ...
                case var2type.BaseType of
                  btU8: tbtdouble(var1^) := Trunc(tbtdouble(var1^)) div tbtu8(var2^);
                  btS8: tbtdouble(var1^) := Trunc(tbtdouble(var1^)) div tbts8(var2^);
                  btU16: tbtdouble(var1^) := Trunc(tbtdouble(var1^)) div tbtu16(var2^);
                  btS16: tbtdouble(var1^) := Trunc(tbtdouble(var1^)) div tbts16(var2^);
                  btU32: tbtdouble(var1^) := Trunc(tbtdouble(var1^)) div tbtu32(var2^);
                  btS32: tbtdouble(var1^) := Trunc(tbtdouble(var1^)) div tbts32(var2^);
              {$IFNDEF PS_NOINT64}    btS64: tbtdouble(var1^) := Trunc(tbtdouble(var1^)) div tbts64(var2^);{$ENDIF}
                  btVariant:  tbtdouble(var1^) := Trunc(tbtdouble(var1^)) div  integer(Variant(var2^));
                  else raise Exception.Create(RPS_TypeMismatch);
                end;
              end;
            btExtended:
              begin
                ...
                case var2type.BaseType of
                  btU8: tbtextended(var1^) := Trunc(tbtextended(var1^)) div tbtu8(var2^);
                  btS8: tbtextended(var1^) := Trunc(tbtextended(var1^)) div tbts8(var2^);
                  btU16: tbtextended(var1^) := Trunc(tbtextended(var1^)) div tbtu16(var2^);
                  btS16: tbtextended(var1^) := Trunc(tbtextended(var1^)) div tbts16(var2^);
                  btU32: tbtextended(var1^) := Trunc(tbtextended(var1^)) div tbtu32(var2^);
                  btS32: tbtextended(var1^) := Trunc(tbtextended(var1^)) div tbts32(var2^);
              {$IFNDEF PS_NOINT64}    btS64: tbtextended(var1^) := Trunc(tbtextended(var1^)) div tbts64(var2^);{$ENDIF}
                  btVariant:  tbtextended(var1^) := Trunc(tbtextended(var1^)) div  integer(Variant(var2^));
                  else raise Exception.Create(RPS_TypeMismatch);
                end;
              end;
            btVariant:
...
begin
  ...
  try //agregado para garantizar que se limpien las listas creadas durante la ejecución, independientemente de que se invoque o no a Cleanup
    repeat
      FStatus := isRunning;
  //    Cmd := InvalidVal;
...
begin
  case Longint(p.Ext1) of
    ...
    10:
      if Stack.GetItem(Stack.Count -2)^.FType.BaseType = btWideString then
        Stack.SetWideString(-1, AnsiUpperCase(Stack.GetWideString(-2))) // Uppercase
      else
        Stack.SetAnsiString(-1, FastUppercase(Stack.GetAnsiString(-2))); // Uppercase
    11:
      if Stack.GetItem(Stack.Count -2)^.FType.BaseType = btWideString then
        Stack.SetWideString(-1, AnsiLowerCase(Stack.GetWideString(-2))) // Uppercase
      else
...
function DoRead_(Caller: TPSExec;Stack: TPSStack;StackPos:integer):boolean;
var
  arr: TPSVariantIFC;
  i:integer;
  s:string;
...
begin
  Result:=true;
  arr:=NewTPSVariantIFC(Stack[integer(Stack.Count)+StackPos],true);
  case arr.aType.BaseType of
    btU8         : begin
      Caller.OnRead(Caller,btU8,i);
      Stack.SetBool(StackPos,tbtU8(i)<>0);     //Boolean
      end;
    btS8         : begin
      Caller.OnRead(Caller,btS8,i);
      Stack.SetInt(StackPos,Tbts8(i));     //ShortInt
      end;
    btU16        : begin
      Caller.OnRead(Caller,btU16,i);
      Stack.SetInt(StackPos,Tbtu16(i));    //Word
      end;
    btS16        : begin
      Caller.OnRead(Caller,btS16,i);
      Stack.SetInt(StackPos,Tbts16(i));    //SmallInt
      end;
    btU32        : begin
      Caller.OnRead(Caller,btU32,i);
      Stack.SetInt(StackPos,Tbtu32(i));    //Cardinal/LongWord
      end;
    btS32        : begin
    ...
    btSingle     : begin
      Caller.OnRead(Caller,btSingle,r);
      Stack.SetReal(StackPos,r);
      end;
    btDouble     : begin
      Caller.OnRead(Caller,btDouble,r);
      Stack.SetReal(StackPos,r);
      end;
    btExtended   : begin
    ...
    btCurrency   : begin
      ...
      Stack.SetCurrency(StackPos,r);
      end;
    else Result:=false;
  end;
end;

function Read_(Caller: TPSExec; p: TPSExternalProcRec; Global, Stack: TPSStack): Boolean;
begin
  if not Assigned(Caller.OnRead) then
    Result := False
  else
...
begin
  ...
  RegisterFunctionName('!' + Uppercase(CS_NOTIFICATIONVARIANTGET), NVarProc, Pointer(1), nil);

  RegisterFunctionName(CS_INTTOSTR, DefProc, Pointer(0), nil);
  RegisterFunctionName(CS_STRTOINT, DefProc, Pointer(1), nil);
  RegisterFunctionName(CS_STRTOINTDEF, DefProc, Pointer(2), nil);
  RegisterFunctionName(CS_POS, DefProc, Pointer(3), nil);
  RegisterFunctionName(CS_COPY, DefProc, Pointer(4), nil);
  RegisterFunctionName(CS_DELETE, DefProc, Pointer(5), nil);
  RegisterFunctionName(CS_INSERT, DefProc, Pointer(6), nil);

  RegisterFunctionName(Uppercase(CS_STRGET), DefProc, Pointer(7), nil);
  RegisterFunctionName(Uppercase(CS_STRSET), DefProc, Pointer(8), nil);
  RegisterFunctionName(Uppercase(CS_UPPERCASE), DefProc, Pointer(10), nil);
  RegisterFunctionName(Uppercase(CS_LOWERCASE), DefProc, Pointer(11), nil);
  RegisterFunctionName(Uppercase(CS_TRIM), DefProc, Pointer(12), nil);

  RegisterFunctionName(Uppercase(CS_LENGTH),Length_,nil,nil);
  RegisterFunctionName(Uppercase(CS_SETLENGTH),SetLength_,nil,nil);
  RegisterFunctionName(Uppercase(CS_LOW),Low_,nil,nil);
  RegisterFunctionName(Uppercase(CS_HIGH),High_,nil,nil);
  RegisterFunctionName(Uppercase(CS_DEC),Dec_,nil,nil);
  RegisterFunctionName(Uppercase(CS_INC),Inc_,nil,nil);

  RegisterFunctionName(Uppercase(CS_SIN), DefProc, Pointer(15), nil);
  RegisterFunctionName(Uppercase(CS_COS), DefProc, Pointer(16), nil);
  RegisterFunctionName(Uppercase(CS_SQRT), DefProc, Pointer(17), nil);
  RegisterFunctionName(Uppercase(CS_ROUND), DefProc, Pointer(18), nil);
  RegisterFunctionName(Uppercase(CS_TRUNC), DefProc, Pointer(19), nil);
  RegisterFunctionName(Uppercase(CS_INT), DefProc, Pointer(20), nil);
  RegisterFunctionName(Uppercase(CS_PI), DefProc, Pointer(21), nil);
  RegisterFunctionName(Uppercase(CS_ABS), DefProc, Pointer(22), nil);
  RegisterFunctionName(Uppercase(CS_STRTOFLOAT), DefProc, Pointer(23), nil);
  RegisterFunctionName(Uppercase(CS_FLOATTOSTR), DefProc, Pointer(24), nil);
  RegisterFunctionName(Uppercase(CS_PADL), DefProc, Pointer(25), nil);
  RegisterFunctionName(Uppercase(CS_PADR), DefProc, Pointer(26), nil);
  RegisterFunctionName(Uppercase(CS_PADZ), DefProc, Pointer(27), nil);
  RegisterFunctionName(Uppercase(CS_REPLICATE), DefProc, Pointer(28), nil);
  RegisterFunctionName(Uppercase(CS_STRINGOFCHAR), DefProc, Pointer(28), nil);
  RegisterFunctionName('!' + Uppercase(CS_ASSIGNED), DefProc, Pointer(29), nil);

  RegisterDelphiFunction(@Unassigned, Uppercase(CS_UNASSIGNED), cdRegister);
  RegisterDelphiFunction(@VarIsEmpty, Uppercase(CS_VARISEMPTY), cdRegister);
  RegisterDelphiFunction(@Null, Uppercase(CS_NULL), cdRegister);
  RegisterDelphiFunction(@VarIsNull, Uppercase(CS_VARISNULL), cdRegister);
  RegisterDelphiFunction(@VarType, Uppercase(CS_VARTYPE), cdRegister);
  {$IFNDEF PS_NOIDISPATCH}
  RegisterDelphiFunction(@IDispatchInvoke, Uppercase(CS_IDISPATCHINVOKE), cdregister);
  {$ENDIF}


  RegisterFunctionName(Uppercase(CS_GETARRAYLENGTH), GetArrayLength, nil, nil);
  RegisterFunctionName(Uppercase(CS_SETARRAYLENGTH), SetArrayLength, nil, nil);

  RegisterFunctionName(Uppercase(CS_RAISELASTEXCEPTION), DefPRoc, Pointer(30), nil);
  RegisterFunctionName(Uppercase(CS_RAISEEXCEPTION), DefPRoc, Pointer(31), nil);
  RegisterFunctionName(Uppercase(CS_EXCEPTIONTYPE), DefPRoc, Pointer(32), nil);
  RegisterFunctionName(Uppercase(CS_EXCEPTIONPARAM), DefPRoc, Pointer(33), nil);
  RegisterFunctionName(Uppercase(CS_EXCEPTIONPROC), DefPRoc, Pointer(34), nil);
  RegisterFunctionName(Uppercase(CS_EXCEPTIONPOS), DefPRoc, Pointer(35), nil);
  RegisterFunctionName(Uppercase(CS_EXCEPTIONTOSTRING), DefProc, Pointer(36), nil);
  RegisterFunctionName(Uppercase(CS_ANSIUPPERCASE), DefProc, Pointer(37), nil);
  RegisterFunctionName(Uppercase(CS_ANSILOWERCASE), DefProc, Pointer(38), nil);

  {$IFNDEF PS_NOINT64}
  RegisterFunctionName(Uppercase(CS_STRTOINT64), DefProc, Pointer(39), nil);
  RegisterFunctionName(Uppercase(CS_INT64TOSTR), DefProc, Pointer(40), nil);
  {$ENDIF}
  RegisterFunctionName(Uppercase(CS_SIZEOF), DefProc, Pointer(41), nil);

  {$IFNDEF PS_NOWIDESTRING}
  RegisterFunctionName(Uppercase(CS_WSTRGET), DefProc, Pointer(42), nil);
  RegisterFunctionName(Uppercase(CS_WSTRSET), DefProc, Pointer(43), nil);
  {$ENDIF}

  RegisterInterfaceLibraryRuntime(Self);
  (* lectura y escritura estándar *)
  RegisterFunctionName(Uppercase(CS_Read),Read_,nil,nil);
  RegisterFunctionName(Uppercase(CS_Show),Show_,nil,nil);
  RegisterFunctionName(Uppercase(CS_ShowAndRead),ShowAndRead_,nil,nil);
  RegisterFunctionName(Uppercase(CS_AnytoString),ToString_,nil,nil);
  (* implementación de los subprogramas para manipulación de listas *)
  RegisterFunctionName(Uppercase(CS_Add),Agregar_,nil,nil);
  RegisterFunctionName(Uppercase(CS_Item),Elemento_,nil,nil);
  RegisterFunctionName(Uppercase(CS_Remove),Borrar_,nil,nil);
  RegisterFunctionName(Uppercase(CS_Count),Cantidad_,nil,nil);
  RegisterFunctionName(Uppercase(CS_SameType),MismoTipo_,nil,nil);
  RegisterFunctionName(Uppercase(CS_whatType),queTipo_,nil,nil);
  RegisterFunctionName(Uppercase(CS_Replace),Reemplazar_,nil,nil);
  RegisterFunctionName(Uppercase(CS_First),Primero_,nil,nil);
...
begin
  ...
  case aType.BaseType of
    ...
    btU8:
      if aType.ExportName = Uppercase(CS_BOOLEAN) then
    ...
    btDouble:
      begin
        if aType.ExportName = Uppercase(CS_TDATETIME) then
...
begin
  ...
  begin
    ...
    begin
      ...
      t := x^[I];
      Assert(t=Ptr,RS_Computation_vmp_fail);
      exit;
    end;
    I := I + 1;
  end;
...
begin
  ...
  begin
    pp := GetPropInfo(cl.FClass.ClassInfo, GetDelphiPropertyName(string(s2)));
    if pp <> nil then
    begin
...
procedure RegisterClassLibraryRuntime(SE: TPSExec; Importer: TPSRuntimeClassImporter);
begin
  SE.AddSpecialProcImport(CS_class, SpecImport, Importer);
end;
...
var
  cn:tbtString;
begin
  cn := tbtString(RTClassName(FastUppercase(tbtString(aClass.ClassName))));
  Result := FindClass(cn);
  if Result <> nil then exit;
  Result := TPSRuntimeClass.Create(aClass, cn);
  FClasses.Add(Result);
end;
...
procedure TPSStack.SetReal(ItemNo: Longint; const Data: real);
var
  val: PPSVariant;
  ok: Boolean;
begin
...
procedure TPSStack.SetChar(ItemNo: Integer; const Data: char);
  procedure PSSetChar(Src: Pointer; aType: TPSTypeRec; var Ok: Boolean; const Val: tbtChar);
  begin
    if (Src = nil) or (aType = nil) then begin Ok := false; exit; end;
    if aType.BaseType = btPointer then
    begin
    ...
    case aType.BaseType of
      btChar: tbtchar(src^) := val;
  {$IFNDEF PS_NOWIDESTRING}    btWideChar: tbtwideChar(src^) := tbtwideChar(AnsiChar(val));{$ENDIF}
      btVariant:
        begin
          try
            Variant(src^) := Val;
