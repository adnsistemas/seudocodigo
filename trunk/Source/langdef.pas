...
{para cambiar la definición del lenguaje, deben cambiarse estas cadenas (preferentemente
generando un recurso para el idioma apropiado) y modificarse uPSUtils.
En uPSUtils se encuentra la definición de las palabras reservadas y el procesador que
interpreta cada porción del scrip para encontrar las palabras reservadas, operadores
y demás elementos sintácticos.}
...
{$ENDIF }
  ...
  CS_TDataSetDesigner = 'TDiseñadorConjuntoDeDatos';
  ...
  CS_TShortCut = 'TAtajo';
  CS_TSmallIntField = 'TCampoEnteroPequeño';
...
{funciones y procedimientos}
  ...
  CS_GetDataSize = 'ObtenerTamañoDeDato';
...
{otros}
  ...
  CS_BlobSize = 'TamañoBlob';
  CS_BlobType = 'TipoBlob';
  CS_BlockReadSize = 'TamañoBloqueLectura';
  CS_bmRead = 'mbLectura';
  ...
  CS_csDesigning = 'ecDiseñando';
  CS_csDesignInstance = 'ecDiseñoInstancia';
  ...
  CS_Designer = 'Diseñador';
  CS_DesignInfo = 'InformacionDeDiseño';
  ...
  CS_External = 'Externo';
  CS_External_Decl = 'Declaración externa';
  CS_faFixed = 'acFijo';
  ...
  CS_poDefaultSizeOnly = 'pTamañoPorOmision';
  CS_poDesigned = 'pDiseñada';
  ...
  CS_POPEXCEPTION = 'Desapilar Excepción';
  ...
  CS_Push = 'Apila';
  CS_PUSHEXCEPTION = 'Apilar Excepción';
...
begin
  ...
  spos := Pos(';',block);
  if spos = 0 then begin //se trata de una porción simple o de una opción
    spos := Pos('|',block);
    if spos > 0 then begin //se trata de una opción
      result := Copy(block,1,spos-1);
    end else begin // porción simple
      spos := Pos(',',block);
...
(*  result := '';
  ...
  if spos = 1 then begin
    ...
    if epos = Length(block) then begin
      ...
      result := nextSinPiece(block);
      Delete(block,Length(block),1);//borro el cierre que había dejado
      exit;
    end else begin
...
{ ¡Se deben mantener ordenados, se hace búsqueda binaria !}
const
  RTClasses_Count = 136;
