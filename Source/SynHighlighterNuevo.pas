...
{$IFDEF SYN_CodeFolding}
  ...
  RE_BlockBegin.Expression := '\b(' + CS_begin + '|' + CS_record  + '|' + CS_class + ')\b';
  RE_BlockBegin.ModifierI := True;

  RE_BlockEnd := TRegExpr.Create;
  RE_BlockEnd.Expression := '\b' + CS_end + '\b';
  RE_BlockEnd.ModifierI := True;

  RE_Code := TRegExpr.Create;
  RE_Code.Expression := '^\s*(' + CS_function+'|'+CS_procedure+')\b';
...
procedure TSynSeudoSyn.AssignmentProc;
var
  prun:integer;
begin
  fTokenId := tkUnknown;
...
begin
  ...
  case FRange of
    ...
    rsBor, rsBorAsm, rsDirective, rsDirectiveAsm:
      ...
      fTokenId := tkUnknown;
      if CS_assignment[1] = fLine[Run] then
        AssignmentProc;
