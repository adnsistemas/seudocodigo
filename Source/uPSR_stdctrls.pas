...
implementation
uses
  langdef, sysutils, classes{$IFDEF CLX}, QControls, QStdCtrls, QGraphics{$ELSE}, controls, stdctrls, Graphics{$ENDIF}{$IFDEF FPC},buttons{$ENDIF};
...
begin
  ...
  begin
    {$IFNDEF PS_MINIVCL}
{$IFNDEF CLX}
    RegisterPropertyHelper(@TCUSTOMLABELCANVAS_R, nil, Uppercase(CS_CANVAS));
{$ENDIF}
    {$ENDIF}
  end;
end;
...
begin
  ...
  begin
    RegisterMethod(@TCUSTOMEDIT.CLEAR, Uppercase(CS_CLEAR));
    RegisterMethod(@TCUSTOMEDIT.CLEARSELECTION, Uppercase(CS_CLEARSELECTION));
    RegisterMethod(@TCUSTOMEDIT.SELECTALL, Uppercase(CS_SELECTALL));
    RegisterPropertyHelper(@TCUSTOMEDITMODIFIED_R, @TCUSTOMEDITMODIFIED_W, Uppercase(CS_MODIFIED));
    RegisterPropertyHelper(@TCUSTOMEDITSELLENGTH_R, @TCUSTOMEDITSELLENGTH_W, Uppercase(CS_SELLENGTH));
    RegisterPropertyHelper(@TCUSTOMEDITSELSTART_R, @TCUSTOMEDITSELSTART_W, Uppercase(CS_SELSTART));
    RegisterPropertyHelper(@TCUSTOMEDITSELTEXT_R, @TCUSTOMEDITSELTEXT_W, Uppercase(CS_SELTEXT));
    RegisterPropertyHelper(@TCUSTOMEDITTEXT_R, @TCUSTOMEDITTEXT_W, Uppercase(CS_TEXT));

    {$IFNDEF PS_MINIVCL}
    RegisterMethod(@TCUSTOMEDIT.COPYTOCLIPBOARD, Uppercase(CS_COPYTOCLIPBOARD));
    RegisterMethod(@TCUSTOMEDIT.CUTTOCLIPBOARD, Uppercase(CS_CUTTOCLIPBOARD));
		RegisterMethod(@TCUSTOMEDIT.PASTEFROMCLIPBOARD, Uppercase(CS_PASTEFROMCLIPBOARD));
		{$IFNDEF FPC}
		RegisterMethod(@TCUSTOMEDIT.GETSELTEXTBUF, Uppercase(CS_GETSELTEXTBUF));
    RegisterMethod(@TCUSTOMEDIT.SETSELTEXTBUF, Uppercase(CS_SETSELTEXTBUF));
		{$ENDIF}{FPC}
    {$ENDIF}
  end;
end;
...
begin
  ...
  begin
    {$IFNDEF CLX}
    RegisterPropertyHelper(@TCUSTOMMEMOLINES_R, @TCUSTOMMEMOLINES_W, Uppercase(CS_LINES));
    {$ENDIF}
  end;
end;
...
begin
  ...
  begin
    RegisterPropertyHelper(@TCUSTOMCOMBOBOXDROPPEDDOWN_R, @TCUSTOMCOMBOBOXDROPPEDDOWN_W, Uppercase(CS_DROPPEDDOWN));
    RegisterPropertyHelper(@TCUSTOMCOMBOBOXITEMS_R, @TCUSTOMCOMBOBOXITEMS_W, Uppercase(CS_ITEMS));
    RegisterPropertyHelper(@TCUSTOMCOMBOBOXITEMINDEX_R, @TCUSTOMCOMBOBOXITEMINDEX_W, Uppercase(CS_ITEMINDEX));

    {$IFNDEF PS_MINIVCL}
    RegisterMethod(@TCUSTOMCOMBOBOX.CLEAR, Uppercase(CS_CLEAR));
    RegisterMethod(@TCUSTOMCOMBOBOX.SELECTALL, Uppercase(CS_SELECTALL));
    RegisterPropertyHelper(@TCUSTOMCOMBOBOXCANVAS_R, nil, Uppercase(CS_CANVAS));
    RegisterPropertyHelper(@TCUSTOMCOMBOBOXSELLENGTH_R, @TCUSTOMCOMBOBOXSELLENGTH_W, Uppercase(CS_SELLENGTH));
    RegisterPropertyHelper(@TCUSTOMCOMBOBOXSELSTART_R, @TCUSTOMCOMBOBOXSELSTART_W, Uppercase(CS_SELSTART));
    RegisterPropertyHelper(@TCUSTOMCOMBOBOXSELTEXT_R, @TCUSTOMCOMBOBOXSELTEXT_W, Uppercase(CS_SELTEXT));
    {$ENDIF}
  end;
end;
...
begin
  ...
  begin
    RegisterPropertyHelper(@TCUSTOMLISTBOXITEMS_R, @TCUSTOMLISTBOXITEMS_W, Uppercase(CS_ITEMS));
    RegisterPropertyHelper(@TCUSTOMLISTBOXITEMINDEX_R, @TCUSTOMLISTBOXITEMINDEX_W, Uppercase(CS_ITEMINDEX));
    RegisterPropertyHelper(@TCUSTOMLISTBOXSELCOUNT_R, nil, Uppercase(CS_SELCOUNT));
    RegisterPropertyHelper(@TCUSTOMLISTBOXSELECTED_R, @TCUSTOMLISTBOXSELECTED_W, Uppercase(CS_SELECTED));

    {$IFNDEF PS_MINIVCL}
    RegisterMethod(@TCUSTOMLISTBOX.CLEAR, Uppercase(CS_CLEAR));
    RegisterMethod(@TCUSTOMLISTBOX.ITEMATPOS, Uppercase(CS_ITEMATPOS));
    RegisterMethod(@TCUSTOMLISTBOX.ITEMRECT, Uppercase(CS_ITEMRECT));
    RegisterPropertyHelper(@TCUSTOMLISTBOXCANVAS_R, nil, Uppercase(CS_CANVAS));
    RegisterPropertyHelper(@TCUSTOMLISTBOXTOPINDEX_R, @TCUSTOMLISTBOXTOPINDEX_W, Uppercase(CS_TOPINDEX));
    {$ENDIF}
  end;
end;
...
begin
  ...
  begin
    RegisterMethod(@TSCROLLBAR.SETPARAMS, Uppercase(CS_SETPARAMS));
  end;
end;
