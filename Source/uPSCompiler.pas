...
{$ENDIF}
  ...
  RPS_OnUseEventOnly = 'Esta función puede utilizarse únicamente desde dentro del evento OnUses';
  RPS_UnableToRegisterFunction = 'Imposible registrar la función %s';
  RPS_UnableToRegisterConst = 'Imposible registrar la constante %s';
  RPS_InvalidTypeForVar = 'Tipo no válido para la variable %s';
  ...
  RPS_CharError = 'Error en carácter';
  RPS_SyntaxError = 'Error de sintaxis';
  RPS_EOF = 'Fin de archivo inesperado';
  RPS_IEndExpected = 'Se esperaba el fin de la instrucción';
  RPS_UnexpectedIEnd = 'Fin de instrucción inesperado';
  ...
  RPS_CloseRoundExpected = 'Se esperaba cierre de paréntesis';
  RPS_TypeMismatch = 'Incoherencia de tipos';
  RPS_InternalError = 'Error interno (%s)';
  RPS_AssignmentExpected = 'Se esperaba asignación';
  ...
  RPS_OpenRoundExpected = 'Se esperaba apertura de paréntesis';
  ...
  RPS_UnSetLabel = 'El rótulo ''%s'' no ha sido establecido';
  RPS_NotInLoop = 'No se encuentra en un bucle';
  RPS_InvalidJump = 'Salto no válido';
  RPS_OpenBlockExpected = 'Se esperaba apertura de corchetes';
  RPS_WriteOnlyProperty = 'Propiedad de sólo escritura';
  RPS_ReadOnlyProperty = 'Propiedad de sólo lectura';
  RPS_ClassTypeExpected = 'Se esperaba un tipo de clase';
  RPS_DivideByZero = 'División por cero';
  RPS_MathError = 'Error matemático';
  RPS_UnsatisfiedForward = 'No se encuentra le implementación de %s';
  RPS_ForwardParameterMismatch = 'Incoherencia en la definición de los parámetros';
  RPS_InvalidNumberOfParameter = 'Número incorrecto de parámetros';
  RPS_UnknownError = 'Error desconocido';
  {$IFDEF PS_USESSUPPORT}
  RPS_NotAllowed = '%s no está permitido en este punto';
  ...
  RPS_FunctionNotUsed = 'La función ''%s'' no se ha utilizado';
  RPS_UnknownHint = 'Consejo desconocido';


  RPS_Warning = 'Advertencia';
  RPS_CalculationAlwaysEvaluatesTo = 'El cálculo siempre devuelve %s';
  RPS_IsNotNeeded =  'No se necesita %s';
  RPS_AbstractClass = 'Construcción de clase abstracta';
...
begin
  try
    Result := False;
    { el bloque de variables está conformado por una o más instrucciones, cada una
    de ellas debe tener un identificador de tipo y al menos uno de variable, opcionalmente
    puede haber más identificadores de variable, a continuación del primero, con algún
    ...
    while not(FParser.CurrTokenId in [CSTII_begin,CSTII_function,CSTII_procedure]) do begin
      ...
      if FParser.CurrTokenId = CSTI_IEnd then begin
        ...
        case sinPieceType(piece) of
          ...
          sptOptional:begin // la parte opcional es siempre una repetición
            ...
            until false;
            // falló la parte opcional, por lo cual tengo el elemento siguiente en el parser
          end;
        end;
      end;
      ...
      try
        ...
        repeat
          ...
          case sinPiecetype(ppiece) of
            ...
            sptOptional: begin // segundo+ parámetros del mismo tipo, o modificador
              tempp := nextSinPiece(ppiece); // contenido de la opción
              if sinPieceType(tempp) <> sptRepeat then begin// modificador
                doModificador(tempp);
                modeSet := True;
              end else repeat //parámetros
                mulpars := tempp;
                subpnum := 0;
                repeat
                  ppiece := nextSinPiece(mulpars); // el primero es alguna forma de separación que debe estar
                  case sinPieceType(ppiece) of
            ...
            sptOption: begin// modificador de parámetro
              doModificador(piece);
              modeSet := True;
            end;
          end;
  ...
  begin
    ...
    try
      ...
      repeat
        ...
        piece := nextSinPiece(block);
        case sinPieceType(piece) of
          sptRepeat:begin //parámetros
            repeticion := piece;
            original := block;
            block := piece;
          end;
          sptRequired:begin //parámetro, separación entre parámetros, o cierre
            item := nextSinPiece(piece);
            if (item = 'paramBlock') and (item = piece) then begin
              if not parseParamBlock(not IsOpt) then begin // si falla, o bien hay un error, o debo interrumpir el procesamiento de esta porción
                if not IsOpt then
                  exit;
                block := '';
                IsOpt := False;
          ...
          sptOptional:begin // puede ser que los parámetros sean opcionales
            isOpt := True;
            ...
            if sinPieceType(repeticion) <> sptRepeat then begin
              ...
              if integer(FParser.CurrTokenId) = StrToIntDef(item,-1) then // cierre (sin parámetros), salir
                break;
            end;
            original := block;
            block := repeticion;
...
{¿por qué???          if Params[c].ExpectedType.BaseType = btChar then
...
begin
  ...
  {Agregar un elemento a la lista, al final de la misma, devuelve el índice del elemento agregado (Cantidad(lst))
  función Agregar(lista lst;inmutable x)entero resultado }
  ...
  {asigna un elemento almacenado en la variable recibida como parámetro, siempre y cuando
  sean del mismo tipo (lo almacenado y la variable), la posición es de 1 a Cantidad(lst).
  ...
  {elimina el elemento de la lista que ocupa la posición indicada
  ...
  {devuelve una cadena con el tipo del parámetro recibido
  ...
  {verifica el tipo de un elemento de la lista, respecto del tipo del parámetro recibido,
  ...
  {quita el último elemento de la lista y devuelve la lista resultante
